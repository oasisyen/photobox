...
(function($, doc){
	...
	*/
	...
	Photobox = function(_options, object, target){
		...
		this.target = target;
		this.selector = $(object.selector || doc);
		
		this.thumbsList = null;
		// only include links which has an Image element as a child
		var images = [];
		this.imageLinks = object.find(target).filter(function(i){
	...
	Photobox.prototype = {
		init : function(){
			...
			// TODO: only generate the thumbStripe once, and listen for any DOM changes on the selector element, if so, re-generate
		...
		open : function(link){
			...
			// TODO: use a class on the overlay...
			...
			if( this.images.length < 2 ){
				...
				caption.find('.counter').hide();
			} else{
				prevBtn.add(nextBtn).show();
			    caption.find('.counter').show();
	...
	APControl = {
		...
		progress : {
			reset : function(){
				...
				console.log(autoplayBtn[0].firstChild);
	...
	function getPrefixed(prop){
		var i, s = doc.createElement('div').style, v = ['ms','O','Moz','Webkit'];
